...
def recognize_thread_func():
    ...
    while is_running:
        ...
        res_top = crop_and_ocr(image, param_clapper.args_top, time_str, frame_index)
        res_middle = crop_and_ocr(image, param_clapper.args_middle, time_str, frame_index)
        res_bottom = crop_and_ocr(image, param_clapper.args_bottom, time_str, frame_index)
        ...
        if frame_index > last_index + 1:
            ...
            def custom_keypress(key, delay1, delay2):
                try:
                    ...
                    stop()
            custom_keypress(key, config.key_delay['clapper'], 0.01)
...
def stop():
    global is_running
    if is_running:
        print('Dừng quét biểu diễn [Phách]')
    is_running = False
