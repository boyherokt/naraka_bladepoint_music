...
def recognize_thread_func():
    ...
    while is_running:
        ...
        res_top = crop_and_ocr(image, param_gong.args_top, time_str, frame_index)
        res_middle = crop_and_ocr(image, param_gong.args_middle, time_str, frame_index)
        res_bottom = crop_and_ocr(image, param_gong.args_bottom, time_str, frame_index)
...
def keypress_thread_func(ctrl):
    ...
    while is_running:
        ...
        if num_top != '':
            if int(num_top) not in param_gong.map_top.keys():
                print(f'[CẢNH BÁO] map_top không có phím tương ứng: {num_top}')
                continue
            key = param_gong.map_top[int(num_top)]
            num = num_top
            ...
            if int(num_middle) not in param_gong.map_middle.keys():
                print(f'[CẢNH BÁO] map_middle không có phím tương ứng: {num_middle}')
                continue
            key = param_gong.map_middle[int(num_middle)]
            num = num_middle
            ...
            if int(num_bottom) not in param_gong.map_bottom.keys():
                print(f'[CẢNH BÁO] map_bottom không có phím tương ứng: {num_bottom}')
                continue
            key = param_gong.map_bottom[int(num_bottom)]
            num = num_bottom
        ...
        # PS: Khoảng cách nhấn của Chiêng khá lớn, nên thử dùng +3 tại đây
        ...
        if key != last_key or frame_index > last_index + 1:
            ...
            def custom_keypress(key, delay1, delay2):
                try:
                    ...
                    stop()
            custom_keypress(key, config.key_delay['gong'], 0.01)
...
def stop():
    global is_running
    if is_running:
        print('Dừng quét biểu diễn [Chiêng]')
    is_running = False
